...
export default function DashboardPage() {
  ...
  const pipelineStages: { status: ApplicationStatus; label: string; bg: string; text: string }[] = [
    { status: 'SAVED', label: 'Saved', bg: 'bg-slate-800/60', text: 'text-slate-300' },
    { status: 'APPLIED', label: 'Applied', bg: 'bg-sky-950/50', text: 'text-sky-300' },
    { status: 'SCREENING', label: 'Screening', bg: 'bg-cyan-950/50', text: 'text-cyan-300' },
    { status: 'INTERVIEW', label: 'Interview', bg: 'bg-indigo-950/60', text: 'text-indigo-300' },
    { status: 'OFFER', label: 'Offers', bg: 'bg-emerald-950/60', text: 'text-emerald-300' },
    { status: 'REJECTED', label: 'Rejected', bg: 'bg-rose-950/50', text: 'text-rose-300' },
    { status: 'WITHDRAWN', label: 'Withdrawn', bg: 'bg-zinc-800/50', text: 'text-zinc-400' },
  ];
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/ai-analyzer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-indigo-600 to-sky-600 hover:from-indigo-500 hover:to-sky-500 text-white shadow-lg shadow-indigo-600/20 transition-all"
            >
          ...
          {pipelineStages.map((stage) => {
            ...
            return (
              <Link
                ...
                className={`glass-card glass-card-hover rounded-2xl p-4 flex flex-col justify-between border ${stage.bg} transition-all`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="glass-card rounded-3xl p-6 flex flex-col justify-between">
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-800">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Scheduled rounds</p>
                  </div>
                </div>
                <Link
                  href="/interviews"
              ...
              <div className="mt-4 space-y-3">
                {loading ? (
                  ...
                  metrics.upcomingInterviews.slice(0, 3).map((inv) => (
                    <div
                      key={inv.id}
                      className="p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800 flex items-start justify-between gap-3"
                    >
                      <div className="space-y-1">
                        ...
                        <span className="inline-block text-[10px] font-semibold uppercase px-2 py-0.5 rounded bg-indigo-950/80 text-indigo-400 border border-indigo-800/60">
                          {inv.roundType}
                        </span>
                      </div>
                      ...
                      {inv.meetingLink && (
                        <a
                          ...
                          className="p-2 rounded-xl bg-sky-500/20 hover:bg-sky-500/30 text-sky-300 border border-sky-500/30 transition-colors shrink-0"
                          title="Join Meeting"
                        >
            ...
            <Link
              href="/interviews"
              className="mt-4 pt-3 border-t border-slate-800 text-center text-xs font-semibold text-slate-400 hover:text-white transition-colors"
            >
              Open Interview Hub →
            </Link>
          </div>
          ...
          <div className="glass-card rounded-3xl p-6 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between pb-4 border-b border-slate-800">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Deadlines & follow-ups</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-3">
                {loading ? (
                  <div className="py-8 text-center text-xs text-slate-500">Scanning opportunities...</div>
                  ...
                  metrics.requiresAttention.slice(0, 3).map((app) => (
                    <Link
                      ...
                      className="p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800 hover:border-amber-500/40 block transition-all group"
                    >
          ...
          <div className="glass-card rounded-3xl p-6 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between pb-4 border-b border-slate-800">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h2 className="text-sm font-bold text-white">Gemini AI Insights</h2>
                    <p className="text-xs text-slate-400">Match score & readiness</p>
                  </div>
                </div>
              </div>
            ...
            <Link
              href="/ai-analyzer"
              className="mt-5 w-full py-2.5 rounded-xl text-xs font-bold text-center bg-gradient-to-r from-sky-600 to-indigo-600 hover:from-sky-500 hover:to-indigo-500 text-white transition-all shadow-md shadow-sky-600/20"
            >
          ...
          <div className="mt-4 divide-y divide-slate-800/60">
            {loading ? (
              ...
              <div className="py-12 text-center">
                <Building className="w-10 h-10 text-slate-600 mx-auto mb-2 opacity-50" />
                <p className="text-sm text-slate-300 font-semibold">No applications yet</p>
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2.5">
                      <Link
                        href={`/applications/${app.id}`}
                        className="font-bold text-sm text-white hover:text-sky-300 transition-colors"
                      >
                        {app.jobTitle}
                      </Link>
