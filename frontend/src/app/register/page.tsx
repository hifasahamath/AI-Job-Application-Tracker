...
import { Sparkles, ArrowRight, Lock, Mail, User, Briefcase, Code, ShieldCheck } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      success('Account created successfully! Welcome to CareerPulse AI.');
      ...
      <div className="w-full max-w-lg relative z-10 my-8">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-400 mt-1">
            Supercharge your job applications with intelligent tracking
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-950 border border-slate-700/80 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-sky-500 transition-colors resize-none"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-3 inline-flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-semibold bg-sky-600 hover:bg-sky-500 text-white shadow-lg shadow-sky-600/25 transition-all disabled:opacity-50"
            >
              {loading ? (
